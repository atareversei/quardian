...
func (t *Translator) loadTranslations() {
	dirs, err := translationFiles.ReadDir(".")
	if err != nil {
		panic("embed panicccccccccc")
	}
	...
	for _, dir := range dirs {
		...
		files, err := translationFiles.ReadDir(lang)
		if err != nil {
		...
		for _, file := range files {
			...
			content, err := translationFiles.ReadFile(fmt.Sprintf("%s/%s", lang, file.Name()))
			if err != nil {
	...
	for _, dir := range dirs {
		...
		lang := dir.Name()
		files, _ := translationFiles.ReadDir(lang)
		...
		for _, file := range files {
			...
			content, _ := translationFiles.ReadFile(fmt.Sprintf("%s/%s", lang, file.Name()))
			var fileTranslations map[string]string
