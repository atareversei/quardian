...
func buildRichError(args RichError) *RichError {
	...
	if args.wrappedError != nil {
		...
		ok := errors.As(args.wrappedError, &nestedRichError)
		if ok {
			re = re.WithError(buildRichError(*nestedRichError))
		} else {
...
func TestRichError_GetMessage(t *testing.T) {
	for _, tc := range getMessageTestCases {
		t.Run(tc.description, func(t *testing.T) {
			if tc.expected != tc.args.GetMessage() {
				t.Errorf("\nExpected: %q\nActual: %q\n", tc.expected, tc.args.GetMessage())
			}
		})
	}
}
