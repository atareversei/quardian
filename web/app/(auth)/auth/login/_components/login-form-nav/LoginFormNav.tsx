...
export function LoginFormNav({ emailQueryParameter }: Props) {
    ...
    <header>
      <nav>
        <Group justify="space-between">
          ...
          <Group gap={0}>
            <Text size="sm">{l.publ.auth.dontHaveAccount}</Text>
            <Button
              component={Link}
              href={Routes.register + queryParameters}
              variant="subtle"
              size="compact-sm"
              type="button"
            >
              {l.publ.auth.registerAccount}
            </Button>
          </Group>
        </Group>
      </nav>
